...
| **Web-дашборд**               | Локальный React + Vite веб-интерфейс (тема Catppuccin) с тем же набором функций, что и TUI: вкладки фильтров, цикл сортировки, группированный/плоский вид, живой поиск, смена статусов, превью PDF и экран прогресса. Автоматически обновляется при изменении `applications.md` |
...
Локальный React + Vite веб-интерфейс, который зеркалит TUI в браузере. Сосуществует с Go TUI — оба читают и пишут один и тот же `applications.md`, так что рассинхронизации не происходит.

```bash
npm run install:web    # разово — установка зависимостей в web/, web/server/, web/client/
npm run serve:web      # dev-режим (Vite HMR на :5174, API на :3333)
npm run build:web      # продакшн-сборка → web/client/dist/
npm run start:web      # продакшн-сервер (отдаёт собранный клиент на :3333)
```

Затем откройте **http://localhost:5174** (dev) или **http://localhost:3333** (prod).

Функциональный паритет с TUI:

- **Экран Pipeline** — 8 табов фильтров, 7 режимов сортировки, группированный/плоский вид, живой поиск, выбор колонок, панель превью с archetype/TL;DR/comp/outcome.
- **Просмотрщик отчётов** — полный markdown-рендеринг (GFM-таблицы + блоки кода), открыть URL вакансии, открыть PDF cover letter, смена статуса inline.
- **Экран прогресса** — воронка, распределение оценок, коэффициенты конверсии response/interview/offer, активность за последние 8 ISO-недель.
- **Горячие клавиши, совместимые с TUI** — `j/k` навигация, `h/l` переключение табов, `s` цикл сортировки, `v` группировка/плоский, `/` поиск, `c` смена статуса, `C` выбор колонок, `Enter` открыть отчёт, `o` URL, `d` PDF, `D` перегенерировать PDF, `p` прогресс, `g/G` начало/конец, `r` обновить, `Esc/q` назад.
- **Живые обновления** — сервер следит за `data/applications.md` через `fs.watch` и отправляет SSE-события; UI автоматически обновляется, когда статус меняется в другом месте (например, из TUI или редактора).
- **Только локально, без авторизации** — привязка к `127.0.0.1`. Все shell-вызовы (`открыть URL`, `открыть PDF`, `перегенерировать PDF`) используют `execFile` с массивами argv и валидируют, что пути остаются внутри репозитория — никакой path-traversal или command-injection поверхности.

Стек: React 18 + TypeScript + Vite 6 + Tailwind CSS (палитра Catppuccin Mocha) + Express 4 бэкенд, переиспользующий `tracker-parse.mjs` и `generate-pdf.mjs`.
...
├── dashboard/                   # Go TUI
├── web/                         # React веб-дашборд (сосуществует с TUI)
...
- **Web-дашборд**: React 18 + TypeScript + Vite 6 + Tailwind CSS + Express 4 (та же тема Catppuccin, авто-обновление через SSE)
