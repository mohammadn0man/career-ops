...
| **Веб-дашборд**               | Локальний React + Vite вебінтерфейс (тема Catppuccin) з тим самим набором функцій, що й TUI: вкладки фільтрів, цикл сортування, груповий/плаский вигляд, живий пошук, зміна статусів, попередній перегляд PDF та екран прогресу. Автоматично оновлюється при зміні `applications.md` |
...
Локальний React + Vite вебінтерфейс, що віддзеркалює TUI у браузері. Співіснує з Go TUI — обидва читають та пишуть той самий `applications.md`, тож ніщо не виходить із синхронізації.

```bash
npm run install:web    # один раз — встановлює залежності web/, web/server/, web/client/
npm run serve:web      # dev-режим (Vite HMR на :5174, API на :3333)
npm run build:web      # продакшн-збірка → web/client/dist/
npm run start:web      # продакшн-сервер (віддає зібраний клієнт на :3333)
```

Потім відкрийте **http://localhost:5174** (dev) або **http://localhost:3333** (prod).

Функціональна паритетність з TUI:

- **Екран Pipeline** — 8 вкладок фільтрів, 7 режимів сортування, групований/плаский вигляд, живий пошук, вибір колонок, панель попереднього перегляду з archetype/TL;DR/comp/outcome.
- **Переглядач звітів** — повне рендерення markdown (GFM таблиці + блоки коду), відкриття URL вакансії, відкриття PDF-супровідного листа, зміна статусу inline.
- **Екран прогресу** — воронка, розподіл оцінок, коефіцієнти конверсії response/interview/offer, активність за останні 8 ISO тижнів.
- **Клавіатурні скорочення сумісні з TUI** — `j/k` навігація, `h/l` перемикання вкладок, `s` цикл сортування, `v` групований/плаский, `/` пошук, `c` зміна статусу, `C` вибір колонок, `Enter` відкрити звіт, `o` URL, `d` PDF, `D` перегенерувати PDF, `p` прогрес, `g/G` початок/кінець, `r` оновити, `Esc/q` назад.
- **Живі оновлення** — сервер спостерігає за `data/applications.md` через `fs.watch` та надсилає SSE-події; UI автоматично оновлюється при зміні статусу деінде (наприклад, у TUI або редакторі).
- **Лише локально, без авторизації** — прив'язується до `127.0.0.1`. Усі виклики shell (`відкрити URL`, `відкрити PDF`, `перегенерувати PDF`) використовують `execFile` з масивами argv та перевіряють, що шляхи залишаються в межах репозиторію — без поверхні для path-traversal чи command-injection.

Стек: React 18 + TypeScript + Vite 6 + Tailwind CSS (палітра Catppuccin Mocha) + Express 4 бекенд, який використовує `tracker-parse.mjs` та `generate-pdf.mjs`.
...
├── web/                         # React веб-дашборд (співіснує з TUI)
...
- **Веб-дашборд**: React 18 + TypeScript + Vite 6 + Tailwind CSS + Express 4 (та сама тема Catppuccin, авто-оновлення через SSE)
