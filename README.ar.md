...
| **لوحة تحكم ويب** | واجهة ويب محلية بـ React + Vite (نمط Catppuccin) بنفس ميزات TUI: تبويبات الفلترة، دورة الترتيب، عرض مجموعات/مسطح، بحث مباشر، تغيير الحالات، معاينة PDF وشاشة التقدم. يتم التحديث تلقائياً عند تغير `applications.md` |
...
واجهة ويب محلية بـ React + Vite تعكس TUI في المتصفح. تتعايش مع Go TUI — كلاهما يقرأ ويكتب في نفس `applications.md`، لذا لا يحدث أي عدم تزامن.

```bash
npm run install:web    # لمرة واحدة — تثبيت الاعتماديات في web/, web/server/, web/client/
npm run serve:web      # وضع التطوير (Vite HMR على :5174، API على :3333)
npm run build:web      # بناء الإنتاج → web/client/dist/
npm run start:web      # خادم الإنتاج (يخدم العميل المبني على :3333)
```

ثم افتح **http://localhost:5174** (تطوير) أو **http://localhost:3333** (إنتاج).

تكافؤ الميزات مع TUI:

- **شاشة Pipeline** — 8 تبويبات فلترة، 7 أوضاع ترتيب، عرض مجموعات/مسطح، بحث مباشر، مُحدِّد الأعمدة، لوحة معاينة مع archetype/TL;DR/comp/outcome.
- **عارض التقارير** — عرض markdown كامل (جداول GFM + كتل الشيفرة)، فتح رابط الوظيفة، فتح PDF خطاب التغطية، تغيير الحالة داخلياً.
- **شاشة التقدم** — أشرطة القمع، توزيع الدرجات، معدلات التحويل response/interview/offer، النشاط في آخر 8 أسابيع ISO.
- **اختصارات لوحة المفاتيح متوافقة مع TUI** — `j/k` للتنقل، `h/l` لتبديل التبويبات، `s` دورة الترتيب، `v` مجموعات/مسطح، `/` بحث، `c` تغيير الحالة، `C` مُحدِّد الأعمدة، `Enter` فتح التقرير، `o` URL، `d` PDF، `D` إعادة توليد PDF، `p` التقدم، `g/G` أعلى/أسفل، `r` تحديث، `Esc/q` رجوع.
- **تحديثات مباشرة** — الخادم يراقب `data/applications.md` باستخدام `fs.watch` ويرسل أحداث SSE؛ تتحدث الواجهة تلقائياً عند تغيير الحالة من مكان آخر (مثلاً TUI أو محرر نصوص).
- **محلي فقط، بدون مصادقة** — يستمع على `127.0.0.1`. جميع استدعاءات shell (`فتح URL`، `فتح PDF`، `إعادة توليد PDF`) تستخدم `execFile` مع مصفوفات argv وتتحقق من أن المسارات تبقى داخل المستودع — لا سطح لهجمات path-traversal أو command-injection.

المكدس التقني: React 18 + TypeScript + Vite 6 + Tailwind CSS (لوحة ألوان Catppuccin Mocha) + خلفية Express 4 تعيد استخدام `tracker-parse.mjs` و `generate-pdf.mjs`.
...
├── web/                         # لوحة تحكم ويب بـ React (تتعايش مع TUI)
│   ├── server/                  # واجهة برمجية Express + SSE عبر fs.watch
