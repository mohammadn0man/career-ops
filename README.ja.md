...
| **Webダッシュボード**    | TUIと同等の機能を持つローカルReact + Vite Web UI（Catppuccinテーマ）: フィルタータブ、ソートサイクル、グループ/フラットビュー、ライブ検索、ステータス変更、PDFプレビュー、進捗画面。`applications.md`変更時に自動更新 |
...
TUIをブラウザで再現するローカルReact + Vite Web UIです。Go製TUIと共存し、両方が同じ`applications.md`を読み書きするため、同期がずれることはありません。

```bash
npm run install:web    # 初回のみ — web/, web/server/, web/client/の依存関係をインストール
npm run serve:web      # 開発モード（Vite HMR :5174、API :3333）
npm run build:web      # プロダクションビルド → web/client/dist/
npm run start:web      # プロダクションサーバー（ビルドされたクライアントを:3333で提供）
```

その後、**http://localhost:5174** (開発) または **http://localhost:3333** (本番) を開きます。

TUIとの機能パリティ:

- **パイプライン画面** — 8つのフィルタータブ、7つのソートモード、グループ/フラットビュー、ライブ検索、カラムピッカー、archetype/TL;DR/comp/outcomeプレビューパネル。
- **レポートビューア** — 完全なMarkdownレンダリング（GFMテーブル + コードブロック）、求人URLを開く、カバーレターPDFを開く、インラインステータス変更。
- **進捗画面** — ファネルバー、スコア分布、response/interview/offerコンバージョン率、直近8 ISO週の活動。
- **TUI互換のキーボードショートカット** — `j/k` ナビゲーション、`h/l` タブ切替、`s` ソート循環、`v` グループ/フラット、`/` 検索、`c` ステータス変更、`C` カラムピッカー、`Enter` レポートを開く、`o` URL、`d` PDF、`D` PDF再生成、`p` 進捗、`g/G` 先頭/末尾、`r` 更新、`Esc/q` 戻る。
- **ライブ更新** — サーバーが`fs.watch`で`data/applications.md`を監視し、SSEイベントをプッシュします。他の場所（例：TUIやエディタ）でステータスが変更されると、UIが自動で更新されます。
- **ローカル専用、認証なし** — `127.0.0.1`にバインド。すべてのシェル呼び出し(`URLを開く`、`PDFを開く`、`PDFを再生成`)はargv配列を伴う`execFile`を使用し、パスがリポジトリ内にあることを検証します — path-traversalやcommand-injectionの攻撃面はありません。

スタック: React 18 + TypeScript + Vite 6 + Tailwind CSS（Catppuccin Mochaパレット）+ `tracker-parse.mjs`と`generate-pdf.mjs`を再利用するExpress 4バックエンド。
...
- **Webダッシュボード**: React 18 + TypeScript + Vite 6 + Tailwind CSS + Express 4（同じCatppuccinテーマ、SSE駆動の自動更新）
